                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200 text-sm">
                      {portalsList.map((portal, idx) => (
                        <tr key={idx}>
                          <td className="px-6 py-4 whitespace-nowrap">{portal.vendorName}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <StyledLink href={`mailto:${portal.email}`}>
                              {getName(portal.firstName, portal.lastName)}
                            </StyledLink>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            {portal.hasStakeholderApproved === true ? (
                              ...
                              <span className="flex items-center justify-center h-5 w-5 text-red-800 font-medium bg-red-100 rounded-full">
                                <XIcon className="h-4 w-4" />
                              </span>
                            ) : (
                              <span className="flex items-center justify-center h-5 w-5 text-gray-500 font-medium bg-gray-100 rounded-full">
...
Home.getLayout = (page: React.ReactChild) => <Layout title="Home">{page}</Layout>
